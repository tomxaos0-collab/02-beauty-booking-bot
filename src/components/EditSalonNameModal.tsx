...
interface EditSalonNameModalProps {
  ...
  onClose: () => void;
  onSaveSalonName: (newName: string) => void;
}
...
export default function EditSalonNameModal({
  ...
  const [salonName, setSalonName] = useState(currentSalonName);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!salonName.trim()) return;
    onSaveSalonName(salonName.trim());
    onClose();
  };
          ...
          <div className="flex items-center justify-between pb-3 border-b border-white/10">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-neutral-400">Название вашей сети салонов</p>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full py-3 px-4 rounded-2xl bg-white/[0.04] border border-white/10 text-white text-base font-extrabold focus:outline-none focus:border-pink-500/60 focus:bg-white/[0.06] transition-all"
              />
            </div>
